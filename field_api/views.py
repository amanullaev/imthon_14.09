...
class ListBookByTimeView(generics.ListAPIView):
    queryset = BookingModel.objects.all()
    serializer_class = BookingSerializer
    permission_classes = (OwnerPermission,)
    def my_endpoint(request):
        ...
        my_model_list = BookingModel.objects.filter(
        Q(bron_qilingan_vaqt__lt=start_time) | Q(bron_qilingan_vaqt__gt=end_time)
          )

        data = {'maydonlar': list(my_model_list.values())
          }
